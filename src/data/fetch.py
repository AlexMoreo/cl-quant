...
def fetch_Webis_cls_10(DOWNLOAD_URL = 'http://www.uni-weimar.de/medien/webis/corpora/corpus-webis-cls-10/cls-acl10-processed.tar.gz',
    ...
    dataset_path = join(dataset_home, 'cls-acl10-processed')
    create_if_not_exist(dataset_home)

    if not exists(dataset_path):
        dataset_path_gz = dataset_home+'.tar.gz'
        print("downloading Webis-CLS1-0 dataset (once and for all) into %s" % dataset_path_gz)
        download_file(DOWNLOAD_URL, dataset_path_gz)
        print("untarring dataset...")
        tarfile.open(dataset_path_gz, 'r:gz').extractall(dataset_home)
