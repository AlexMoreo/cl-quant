...
def WebisCLS10_Upper_task_generator(dataset_home='../datasets/Webis-CLS-10'):
    ...
    for target_lan in ['de', 'fr', 'jp']:
        for domain in ['books', 'dvd', 'music']:
            print('Loading Webis-CLS-10 task '+'{}{}-{}{}'.format(target_lan,domain,target_lan,domain).upper())

            tr_t_docs, tr_t_labels = list(zip(*documents[target_lan][domain]['train.processed']))
            te_t_docs, te_t_labels = list(zip(*documents[target_lan][domain]['test.processed']))

            tfidf = TfidfVectorizer(sublinear_tf=True, token_pattern=patt, min_df=1)
    ...
    for s_domain in ['books', 'dvd', 'music']:
        for target_lan in ['de', 'fr', 'jp']:
            for t_domain in ['books', 'dvd', 'music']:
                ...
                print('Loading Webis-CLS-10 task '+'{}{}-{}{}'.format(source_lan,s_domain,target_lan,t_domain).upper())
