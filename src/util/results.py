...
class Result:
    default_column_names = ['dataset', 'method', 'approach', 'metric', 'score']
    ...
    def pivot(self, index=['dataset','method','approach','metric'], values='score', aggfunc=np.mean, grand_totals=False):
        pv = pd.pivot_table(self.df, values=values, index=index, columns=['metric'], aggfunc=aggfunc)
        print(pv)
        if grand_totals:
            print('\nGrand Totals')
